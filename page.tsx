...
function BackgroundPaths({ }: { title?: string }) {
  return (
    <div className="absolute inset-0 pointer-events-none">
...
export default function LandingPage() {
  ...
  const [lastFocus, setLastFocus] = useState<"amount" | "converted">("amount")
  ...
  function handleNumberKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    const allowed = ["Backspace", "ArrowLeft", "ArrowRight", "Delete", "Home", "End", "."]
  ...
  function isCrypto(symbol: string) {
    return ["btc", "eth", "sol", "doge", "ada"].includes(symbol)
  }

  function isFiat(symbol: string) {
    return ["usd", "eur", "gbp", "jpy", "cny"].includes(symbol)
  }
  ...
  function toUsd(amount: number, symbol: string) {
    if (isCrypto(symbol)) return amount * cryptoToUsd[symbol as keyof typeof cryptoToUsd]
    if (isFiat(symbol)) return amount * fiatToUsd[symbol as keyof typeof fiatToUsd]
    return 0
  }

  function fromUsd(amount: number, symbol: string) {
    if (isCrypto(symbol)) return amount / cryptoToUsd[symbol as keyof typeof cryptoToUsd]
    if (isFiat(symbol)) return amount / fiatToUsd[symbol as keyof typeof fiatToUsd]
    return 0
  }
  ...
  const faqTitleClass = isLight ? "text-[#FFD700]" : ""
  const faqDescClass = isLight ? "text-white" : ""
  const faqQuestionClass = isLight ? "text-[#FFD700]" : ""
  const faqAnswerClass = isLight ? "text-white" : ""
      ...
      <header className={`sticky top-0 z-50 w-full backdrop-blur-lg transition-all duration-300 ${isScrolled ? "bg-background/80 shadow-sm" : "bg-transparent"}`}>
        <div className="container flex h-16 items-center justify-between">
          <div className="flex items-center gap-2 font-bold">
            <CustomLogo text="ChainSwitch" letter="C" bgColor="bg-gradient-to-br from-cyan-400 to-cyan-300 dark:from-red-400 to-red-300" />
          </div>
          <nav className="hidden md:flex gap-8">
            ...
            <Link href="#price-conversion" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">Price Conversion</Link>
        ...
        {mobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="md:hidden absolute top-16 inset-x-0 bg-background/95 backdrop-blur-lg border-b">
            <div className="container py-4 flex flex-col gap-4">
              ...
              <Link href="#price-conversion" className="py-2 text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>Price Conversion</Link>
              <Link href="#about" className="py-2 text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>About</Link>
              <Link href="#faq" className="py-2 text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>FAQ</Link>
              <div className="flex flex-col gap-2 pt-2 border-t"></div>
            </div>
          </motion.div>
        )}
      </header>
      <main className="flex-1">
            ...
            <div className="container px-4 md:px-6 relative">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center max-w-3xl mx-auto">
                ...
                <div className="flex flex-col gap-4 mt-4 w-full max-w-md mx-auto">
                  ...
                  <Link href="/wallet">
                    <Button size="lg" className="rounded-full h-12 px-8 text-base bg-cyan-400 hover:bg-cyan-500 dark:bg-red-400 dark:hover:bg-red-500 text-black dark:text-white">
                      Start Now
        ...
        <section id="features" className="w-full py-20 md:py-32">
          <div className="container px-4 md:px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
              <Badge className="rounded-full px-4 py-1.5 text-sm font-medium" variant="secondary">Features</Badge>
        ...
        <CustomBackground>
          <section id="price-conversion" className="w-full py-20 md:py-32 bg-muted/30 relative overflow-hidden">
            <div className="container px-4 md:px-6 relative">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
                <Badge className="rounded-full px-4 py-1.5 text-sm font-medium" variant="secondary">Price Conversion</Badge>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-3 gap-4">
                          <div className="col-span-2">
                            <Input type="text" placeholder="Amount" className="h-12" id="amount-input" onFocus={() => setLastFocus("amount")} onKeyDown={handleNumberKeyDown} />
                          </div>
                          <div>
                            <select className="w-full h-12 rounded-md border border-input bg-background px-3 py-2" id="from-currency">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div className="col-span-2">
                            <Input type="text" placeholder="Converted amount" className="h-12" id="result-input" onFocus={() => setLastFocus("converted")} onKeyDown={handleNumberKeyDown} />
                          </div>
                          <div>
                            <select className="w-full h-12 rounded-md border border-input bg-background px-3 py-2" id="to-currency">
                        ...
                        <Button className="w-full rounded-full h-12 dark:bg-red-400 dark:hover:bg-red-500 bg-cyan-400 hover:bg-cyan-500 text-black" onClick={() => {
                          try {
                            ...
                            const amountVal = Number.parseFloat(amountInput.value)
                            const resultVal = Number.parseFloat(resultInput.value)
                            if (lastFocus === "amount") {
                              if (isNaN(amountVal)) {
                              ...
                              const usdValue = toUsd(amountVal, fromCurrency)
                              const final = fromUsd(usdValue, toCurrency)
                              resultInput.value = parseFloat(final.toFixed(8)).toString()
                            } else {
                              if (isNaN(resultVal)) {
                                alert("Please enter a valid number")
                                resultInput.focus()
                              ...
                              const usdValue = toUsd(resultVal, toCurrency)
                              const final = fromUsd(usdValue, fromCurrency)
                              amountInput.value = parseFloat(final.toFixed(8)).toString()
                            }
                          } catch (error) {
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
                  <Card className="h-full overflow-hidden border-border/40 bg-gradient-to-b from-background to-muted/10 backdrop-blur">
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-4">
                        <div className="mb-4">
                          <Input type="text" placeholder="Enter wallet address to track" className="h-12 mb-2" id="wallet-address-input" />
                          <Button className="w-full rounded-full mb-4 dark:bg-red-400 hover:bg-dark-500 bg-cyan-400 hover:bg-white-500 text-black" onClick={() => {
                        ...
                        <Button className="w-full rounded-full dark:bg-red-400 dark:hover:bg-red-500 bg-cyan-400 hover:bg-cyan-500 text-black">View All Assets</Button>
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
                        ...
                        <tbody>
                          {[
                            ...
                            <tr key={i} className="border-b border-border/40 hover:bg-muted/30">
                              ...
                              <td className={`text-right p-3 ${coin.change.startsWith("+") ? "text-green-500" : "text-red-500"}`}>
                                {coin.change}
                              </td>
        ...
        <section id="about" className="w-full py-20 md:py-32">
          <div className="container px-4 md:px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
              <Badge className="rounded-full px-4 py-1.5 text-sm font-medium" variant="secondary">About Us</Badge>
            ...
            <div className="grid gap-12 md:grid-cols-2 items-center">
              <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
                <div className="rounded-xl overflow-hidden shadow-lg">
                  <Image src="https://images.unsplash.com/photo-1621761191319-c6fb62004040?q=80&w=1000" width={600} height={400} alt="Cryptocurrency visualization" className="w-full h-auto object-cover" />
                </div>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} className="space-y-6">
                ...
                <p className="text-muted-foreground">
                  Founded in 2023, our platform was built with a simple goal: to make cryptocurrency accessible to everyone, regardless of their technical background or investment experience.
                </p>
                <p className="text-muted-foreground">
                  We believe that the future of finance is decentralized, and we're committed to providing the tools and resources needed to navigate this new landscape with confidence.
                </p>
                <div className="space-y-4">
        ...
        <CustomBackground>
          <section id="faq" className="w-full py-20 md:py-32">
            <div className="container px-4 md:px-6">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
                <Badge className={`rounded-full px-4 py-1.5 text-sm font-medium ${faqTitleClass}`} variant="secondary">FAQ</Badge>
                <h2 className={`text-3xl md:text-4xl font-bold tracking-tight ${faqTitleClass}`}>Frequently Asked Questions</h2>
                <p className={`max-w-[800px] text-muted-foreground md:text-lg ${faqDescClass}`}>Find answers to common questions about our platform.</p>
              </motion.div>
                  ...
                  {[
                    ...
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: i * 0.05 }}>
                      <AccordionItem value={`item-${i}`} className="border-b border-border/40 py-2">
                        <AccordionTrigger className={`text-left font-medium hover:no-underline ${faqQuestionClass}`}>{faq.question}</AccordionTrigger>
                        <AccordionContent className={`text-muted-foreground ${faqAnswerClass}`}>{faq.answer}</AccordionContent>
                      </AccordionItem>
                    </motion.div>
                  ))}
                </Accordion>
        ...
        <section className="w-full py-20 md:py-32 bg-gradient-to-br from-cyan-900 to-cyan-900 dark:from-red-900 dark:to-red-900 text-white dark:text-black relative overflow-hidden">
          ...
          <div className="container px-4 md:px-6 relative">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-col items-center justify-center space-y-6 text-center">
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight">Ready to Track Your Crypto Portfolio?</h2>
              <div className="flex flex-col w-full max-w-md gap-4 mt-4">
                <div className="bg-white/20 p-6 rounded-xl border border-white/30 backdrop-blur-sm">
                  ...
                  <Input type="text" placeholder="Enter your wallet address" className="rounded-full h-12 bg-white/30 border-white/20 text-black placeholder:text-black/60 mb-4" />
                  <Button size="lg" className="w-full rounded-full h-12 px-8 text-base bg-black hover:bg-black/80 text-white">
                    Start Now
                    <ArrowRight className="ml-2 size-4" />
                  </Button>
                </div>
      ...
      <footer className="w-full border-t bg-background/95 backdrop-blur-sm">
        <div className="container flex flex-col gap-8 px-4 py-10 md:px-6 lg:py-16">
          <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3">
            <div className="space-y-4">
              <CustomLogo text="ChainSwitch" letter="C" bgColor="bg-gradient-to-br from-red-400 to-red-300" />
              <p className="text-sm text-muted-foreground">
                Track cryptocurrency prices, convert between coins and fiat currencies, and checking your wallet transactions, all in one place.
              </p>
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-2 text-sm">
                ...
                <li>
                  <Link href="#price-conversion" className="text-muted-foreground hover:text-foreground transition-colors">
                    Price Conversion
                  </Link>
                </li>
                <li>
